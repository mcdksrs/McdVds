...
net user McdKsr mcdksr01 /add >nul
net localgroup administrators administrator /add >nul
echo Hepsi Tamam Şimdi Bağlan
echo IP:
tasklist | find /i "ngrok.exe" >Nul && curl -s localhost:4040/api/tunnels | jq -r .tunnels[0].public_url || echo "Discord McdKsr#0001"
echo User: McdKsr
echo Pass: mcdksr01
